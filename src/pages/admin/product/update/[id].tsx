...
const UpdateProductPage: React.FC<UpdateProductPageProps> = ({ product }) => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/product/productsingle/${updatedProduct.id}`, {
        method: 'PUT',
        headers: {
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/product/productsingle/${id}`);
